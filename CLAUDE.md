...
**一律用画布模式挂载，不用视频模式（已定，已实测）。** JASSUB 的视频模式按
`<video>` 的**内容矩形**定位自己的画布（`_getElementBoundingBox`），于是**字幕永远
画不进黑边**——补边到 16:9 之后预览纹丝不动，根子就在这里。画布模式把画布铺满
整个画面区，画布尺寸直接取**这份 ASS 自己声明的 `PlayRes`**（`lib/jassub.parsePlayRes`），
补边与不补边走同一条路。代价是时钟要自己推：画布模式没有 rVFC 自动重绘，
每帧调 `renderAt`（`SubtitleOverlay` 内部已排队去重）。

**画布的 CSS 盒子必须由布局把比例锁死（已实测）。** 这是画布模式的第二笔代价，
比时钟那笔隐蔽得多：JASSUB **不碰 canvas 的 CSS 尺寸**（`resize()` 里设 CSS 宽高
那段有 `if (this._video)` 守着），它只把位图按 PlayRes 比例内缩，之后交给 CSS 去拉伸。
于是盒子比例一旦不等于 PlayRes，**字幕就被拉伸**，而同一个盒子里的 `<video>` 有
`object-fit: contain` 顶着、画面纹丝不动——用户看到的现象是"只有字幕会跟着窗口变形"。
视频模式没这个毛病，是因为那时 JASSUB 自己把 canvas 摆到视频的内容矩形上；
改画布模式等于把这层保护换成了自己的责任。

三个舞台的画面块都允许被上下游布局压扁（编辑侧 `flex: 0 1 auto`、导出侧
`flex: 1 1 auto`、样式侧 `flex: 1 1 auto`），所以画布盒子只能自己内缩：宽度取
`min(100cqw, 100cqh × 画幅比)`，高度交给 `aspect-ratio` 推导，父级声明成 size 容器。
**已被验证是错的写法**：`width: 100% + max-height: 100% + aspect-ratio`，指望宽高比
把高度上限转成宽度上限——chromium 与 WebKit 实测都**不会**，`width` 是明确值时
只有高度被截断、宽度纹丝不动。回归基线 `frontend/scripts/verify-preview-aspect.mjs`
（60 个"挂法 × 画幅 × 窗口"组合形变恒为 1.000，旧写法作阳性对照最大形变 9.15×，
并自检每个挂法真的被压扁过——盒子没压扁的话"没形变"什么也证明不了）。

**画布尺寸必须从 ASS 里读，不许前端另算一遍。** 补边规则（补哪一侧、长边封顶、
取偶）只在后端 `render.geometry.plan_canvas` 里有一份；前端照抄一份必然漂移，
而症状是预览与成片版面不一致——正是 §5.12 要杜绝的那种"到成片里才暴露"的分叉。
这与字体链契约（`PREVIEW_FONTS_TAG`）是同一个机制、同一条理由，也同样不违反
§4.1：读的是**渲染契约**，不是工程状态。
...
| 项 | 规格 | 由来 |
|---|---|---|
| 字号 | **版面锚点高度**的 **7.5%**（16:9 4K → 162px） | 5.2% 实测偏小，卡拉OK 字幕要压在任意画面上仍醒目。锚点见下方「画幅」 |
| 描边 / 阴影 | 字号的 **5.5% / 2.2%**，随字号自适应 | 固定 3px 在 4K 下细到几乎不可见 |
| 边距 | 左右按**宽度** 4.5%、上下按**高度** 5.5% | 边距是方向性的量，各取自己那一维，不掺锚点 |
...
上面这些数值全是在 16:9 上校准的，而**"字的比例不对"有三个互不相同的来源，
症状却一模一样**。判据与计算集中在 `render/geometry.py`，只允许有这一份：

| 来源 | 机制 | 症状 |
|---|---|---|
| **`PlayRes` ≠ 实际帧尺寸** | libass 按 `帧高/PlayResY` 缩字号、按 `帧宽/PlayResX` 缩坐标 | 两个比例不等时**字号不变而坐标被压**，行顶出画面或缩成一团 |
| **字号只锚高度** | 字号同时决定"读起来多大"（看高度）与"一行放几个字"（看宽度） | 4:3 上字相对宽度大 33%、竖屏大 3.16 倍；一行放不下就走 `\fscx` 压扁，那是字真的变形 |
| **非方形像素（SAR ≠ 1）** | 成片按编码尺寸出，播放器再横向拉一次 | 字连同画面一起被拉。**画面看着是正常的**，所以极易误判成字体问题 |

由此三条硬性规则：

- **工程里的 `video_width / video_height` 一律是方形像素的显示尺寸**，与 ASS 的
  `PlayRes`、前端 `<video>.videoWidth`（HTML 规范就是显示尺寸）同一口径。
  下载与本地导入都必须回写它——**曾经只有本地导入回写**，下载来的片子把工程留在
  默认 1920×1080 上，16:9 的源靠"比例恰好相同"蒙混过关，非 16:9 的当场变形。
  存量工程由 `GET /api/media/probe/{id}` 顺手纠正，走 `update_derived` 不占撤销格。
- **字号锚在 `layout_ref_height(w, h)`：以画面高度为准，但受"一行放得下"封顶。**
  `ref = min(h, w × (1−2×0.045) ÷ (20 × 0.694 × 0.075))`，四个因子分别是左右边距比、
  §5.8 的单行字数上端、§5.7 实测的全角 advance 比、§8.5 的字号比。
  **16:9 上它恒等于高度**，既有默认值与已验证的成片逐像素不变——这是回归风险为零的
  全部依据。**已被验证是错的写法**：取内接 16:9 框的高度（`min(h, w×9/16)`）。
  那等于把字号一路绑死在宽度上，4:3 会被白白缩掉 25%（它一行本来就放得下 23 个
  全角字），1:1 会缩到画面高的 4.2%——小到不像卡拉OK 字幕，而真正的毛病
  （PlayRes 对不上）根本不在这儿。封顶只在真放不下时才生效：1:1 与竖屏才会压。
- **烧录前的画布变换（像素归方 / 补边）必须排在 `ass=` 之前**。放后面就成了
  "先烧字再缩放"，字会跟着画面一起被缩进黑边。尺寸与 pad 偏移**全部取偶**，
  `yuv420p` 下奇数会让 ffmpeg 直接拒绝，而报错内容与本功能毫无关系。

**补边到 16:9 是画幅选择，不是修正**（导出选项 `pad_to_16_9` / CLI `--pad-16-9`）：
不补的路径同样是对的。补边解决的是另一件事——宽画面上下补边后歌词落在黑边里
不压画面。竖屏源补 16:9 画面只占中间一条，那是画幅本身的结果，能做的只是别让
画布无限膨胀（长边封顶 3840，超了整体缩小）。

**补边开关必须同时作用于预览**（store 的 `padTo169`，与 `guideEnabled` 同一性质）。
**已被验证是错的做法**：把它做成导出面板的本地 state、只随烧录请求发出去。
补边改的是画布尺寸，也就是 ASS 的 `PlayRes`——字号相对画面的大小、边距、
上下行错开、一行放几个字全都跟着变；勾了却看不到，用户只能靠导出成片来确认版面，
而那正是这个工具要消灭的东西。预览侧的落地方式见 §5.9「画布模式」。

实测对照（1440×1080 SAR 4:3，按播放器显示的样子截帧）：修前同一句字横向被拉
1.33 倍（正好是 SAR），修后比例正常；四种组合（4:3/anamorphic × 补边/不补）
烧录后的帧尺寸与计划画布逐一相等。
...
# 非 16:9 的源补黑边到 16:9 再烧（不加也对：版面会按实际画幅自适应，见 §8.5「画幅」）
… --pad-16-9
```
