...
- **开唱引导点与它提示的那句歌词同时浮现**（同一个 `show`、同一个淡入时长），
  然后**自右向左**依次熄灭，最左那点消失即开唱。
  点亮起后会先静止一段（歌词提前 4.2s，而熄灭只占最后 n 拍），这是刻意的：
  点先是"这段要开始了"的准备标记，最后几拍才变成倒计时。
  让点晚于歌词才亮虽然能去掉静止期，但那就不是"同步出现"了，两个元素先后冒出来更碎。
