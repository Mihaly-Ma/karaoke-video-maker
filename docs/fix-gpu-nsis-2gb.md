...
v0.2.1 出包全过——dmg 419 MB、Windows CPU 版 exe 263 MB、GPU 免安装 zip 2.1 GB——
却红在**上传**：
...
GitHub Release 的**单个 asset** 上限也是 2 GiB。两个 2 GB 是巧合而不是同源：一个是
makensis 的有符号整数溢出，一个是 GitHub 的上传策略。所以代码里是两个常量，
`_INSTALLER_HARD_LIMIT` 与 `_RELEASE_ASSET_LIMIT`，**故意不合并**。

**换更狠的压缩救不了**：solid LZMA2 实测比 0.53，换算下来约 1.9–2.0 GB，过线但余量
近乎为零，torch 下次一涨就原地重演。而 Release 的总大小与文件数都不限，官方也把
「拆成多个 asset」列为标准做法——所以出路是分卷。

`build_portable_zip()` 因此改成**按写出的实际字节滚动切分**，每卷目标 1.5 GB：

- **每一卷都是自成一体的普通 zip**，不是 `.7z.001` 那种分卷。用户把几卷都解压到
  同一位置合并成一个文件夹即可，不需要第三方工具——免安装形态的意义就是少一道
  安装门槛，再加一道解压门槛等于白做。
- **按输出大小切，不按内容规则切**。「把 `_internal/torch/lib` 单独拿出来」今天成立，
  明天 torch 换个目录就悄悄失效，而失效的表现正是某一卷又超限。按大小切的话
  torch 再涨只是多出一卷。
- 说明文件**每卷放一份**：只解压到第二卷的人也得看得到怎么用。
- CI 的收拢步骤必须**保留原始文件名**（`portable-1of2.zip`）。按扩展名重命名会让两卷
  同名互相覆盖，而发布里少一卷的症状是用户解压完缺 dll。

配套还有一条运维教训：`gh release create` 是**先建 release 再传 asset**，它在第三个
文件上失败，于是远端留下一个带两个 asset 的 draft。重跑同一个 tag 会撞「release 已
存在」——重试前得先把那个 draft 删掉。
