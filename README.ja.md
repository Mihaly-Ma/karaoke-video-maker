...
```
YouTube リンク ──▶ ダウンロード (yt-dlp) ──▶ 音声抽出 ──▶ プロキシ動画（H.264、編集専用）
                                                │
                                                ▼
...
- **フォントの同梱**——スタイルのステップでは OS にインストール済みのフォントから選択
  できますが、フォントファイル自体を同梱・自動取得することはありません。なお、フォント
  選択画面が示すのはその書体が **CJK に対応しているかどうか**だけです。楽曲ごとの
  字形カバレッジを調べる API（`POST /api/fonts/coverage`）はバックエンドに存在しますが
  画面からはまだ呼んでいないため、**特定の漢字が字形として欠けていても書き出し前には
  気づけません**。
