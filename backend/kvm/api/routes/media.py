...
from kvm.media.download import _ffprobe_bin, heal_video_size, project_media_dir
...
def probe_all_media(
    ...
    # 与 `GET /api/projects/{id}` 共用同一个纠正实现。**真正兜住用户的是那一处**
    # ——本端点目前前端没有调用点，只在这里挂的话自愈就是死代码（§2.6）。
    # 这里保留一份是因为它语义上就该发生：探完媒体正是"知道了实际尺寸"的时刻。
    heal_video_size(store, project)
