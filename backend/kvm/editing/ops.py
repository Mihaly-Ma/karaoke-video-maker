...
def _colors(unsung: tuple[str, str], sung: tuple[str, str]) -> PaletteDTO:
    """四色，参数是 `(填充, 描边)` 的 `#RRGGBB` 二元组。"""
    return PaletteDTO(
        name="",
...
def _swap_scheme(name: str, description: str, *, base: str, accent: str) -> PaletteScheme:
    """**对调机制**：未唱 = 底色填充 + 声部色描边；已唱 = 两者原样对调。

    `base` 是家族底色（中性或家族色调），`accent` 是声部色。

    三件事被这一个机制同时解决：

    1. **声部身份始终由 accent 承载，唱之前就看得见**——它在未唱态占着描边。
    2. **开唱瞬间填充与描边整体反转**，实测感知差（CIELAB ΔE，填充 + 描边合计）
       达 **153–193**，是"只翻填充"那一档（75–128）的 1.5–2 倍。
    3. **两个状态的填充/描边对比度完全相同**（对调不改变这对颜色，只交换角色），
       所以可读性判据只需成立一次，不会出现"未唱能读、已唱糊掉"。

    还有一条不显然但很有用的推论：**底色为白时，"填充 vs 描边对比度 ≥ 4.5"
    与"描边 vs 纯白背景对比度 ≥ 4.5"是同一个算式**。也就是说既有的可读性判据
    顺带把"彩色描边压在雪景/逆光这类亮底上会不会糊"这件事也框住了——
    这正是彩色描边最让人担心的场景，不必再另立一条。

    代价要如实记：对调后**已唱填充是深色**（accent 必须够暗才满足对比度），
    压在纯黑画面上填充本身只有 2.8–3.9 的对比度；此时靠的是翻上来的白描边
    （21.0）。所以判据必须是"填充与描边**至少有一个**能从底上跳出来"，
    而不是"填充必须跳出来"——按后者写会把这个本来可读的组合误杀。
    """
    return PaletteScheme(
        name=name,
        description=description,
        builtin=True,
        colors=_colors((base, accent), (accent, base)),
    )


def _shift_scheme(
    name: str, description: str, *, base: str, accent: str, bright: str, deep: str
) -> PaletteScheme:
    """**只翻填充机制**：未唱 = 底色填充 + 声部色描边；已唱 = 亮声部色填充 + 深声部色描边。

    声部色贯穿三个明度：未唱描边取中等明度（看得出色相、认得出声部），
    已唱填充取高明度（醒目），已唱描边取低明度（把亮填充托住）。

    与对调机制的取舍：**描边在两个状态下都是深色**，因此亮画面上两个状态都稳；
    代价是未唱↔已唱的感知差只有对调的一半左右（实测 75–128 vs 153–193）。
    两种机制并存是有意的——不同画面与不同曲子该选哪种，只有用户知道。
    """
    return PaletteScheme(
        name=name,
        description=description,
        builtin=True,
        colors=_colors((base, accent), (bright, deep)),
    )
...
def builtin_palette_schemes() -> list[PaletteScheme]:
    ...
    ## 三条硬性判据（`tests/test_palette_schemes.py` 逐项验，改表会被挡住）

    卡拉OK 字幕压的是 **MV 画面**，不是纯色底，而且对唱曲的演唱者要在轮到自己
    **之前**就知道下一句归谁。由此：

    1. **任意底上都读得清**。填充与描边的 WCAG 对比度 ≥ 4.5；且在纯黑与纯白
       两个极端底上，填充与描边**至少有一个**对底的对比度 ≥ 4.5。
       写成"至少一个"而不是"填充必须"是有原因的，见 `_swap_scheme` 的代价那一段。
    2. **未唱态要能区分声部**（最高优先级）。同一家族内的不同变体是给不同声部
       准备的，它们的**未唱描边**必须差得开（ΔE ≥ 25），且描边色度 ≥ 18
       ——太暗太灰的"深褐""墨蓝"在描边宽度下与近黑分不出来，那是**假装有区别**。
    3. **未唱态要能区分家族**。家族底色（未唱填充）两两 ΔE ≥ 25。

    ## 区分度压在哪个通道上（这是本表的分配逻辑）

    | 承载什么 | 放在哪 | 为什么 |
    |---|---|---|
    | 家族身份 | **未唱填充**（底色） | 面积最大、明度最高，一眼就分得出 |
    | 声部身份 | **未唱描边的色相** | 填充已被家族占用；而描边贴着字形轮廓，逐字可见 |
    | 唱没唱 | 填充与描边**同时**变 | 双层 + 渐进 clip 天生支持，变化最显眼 |

    三者互相挤压色彩空间：描边要够暗才保得住可读性，又要够饱和才认得出声部，
    而底色还得给家族留出明度层级。上面这张分配表就是挤压之后的解。

    ## 曾经写在这里的一条**错误**规则

    > ~~未唱色在同一套内对所有声部一致，只有已唱色分声部；否则同屏两个声部
    > 在开唱之前看起来就像两首歌的歌词。~~

    **这条对卡拉OK 是错的。**整个产品为"让人有时间准备"花了大力气——提前入场
    2.0s、段落首行提前 4.2s、开唱引导点踩着真实拍点倒数（§8.5）——如果未唱态
    两个声部长得一模一样，"这句是不是我的"要等扫色开始才知道，那时已经晚了，
    前面所有提前量都白给。日本点唱机的对唱模式本来就是**未唱时就分色**的。
    原来那条顾虑（"看起来像两首歌"）是真问题，但解法是**同族而可辨**：
    同一家族共享底色与明度层级，只在描边色相上分开。

    ## 家族覆盖的画面类型

    亮画面要有深字方案（白字在雪景/白背景上会整片消失），暖画面要有暖方案
    （冷色压在暖底上发脏）。CLAUDE.md §5.8 记着默认那套「白字黑边 → 红字白边」
    的来历，也记着它**不是日本业界标准、只是同人圈一位博主的实践总结**——
    所以这里不做它的变体集，而是把它作为「经典白 · 红」保留一席。
    """
    return [
