...
- **人声分离的模型权重是首次用到时才下载的**（84MB～640MB），但绝不静默下载：
  自检只报告缺什么，自己一个字节也不下。引导声的音高模型（CREPE）随 Python 包一起
  装好，**运行时不联网**。
...
```bash
python3 scripts/package.py   # 前端构建 → PyInstaller → Tauri 出包
```

产物在 `src-tauri/target/release/bundle/`（macOS：1.1 GB 的 `.app`、352 MB 的 `.dmg`）。
**窗口本身就是应用，不需要再开浏览器。** 首次启动时它会把带 libass 的 ffmpeg 下载到
自己的私有目录（`~/.karaoke-video-maker/bin`）：有进度条、校验和写死在代码里、支持断点续传，
卸载就是删掉那个目录。装不上时启动页会写清楚缺什么、为什么、能怎么办，**不会一直转圈**。

产物**没有签名**。macOS 上从 `.dmg` 装的应用首次打开会被 Gatekeeper 拦下：
右键 →「打开」→「打开」，或
`xattr -dr com.apple.quarantine "/Applications/Karaoke Video Maker.app"`。
Windows 的 SmartScreen 点「更多信息」→「仍要运行」。自己在本机构建的产物不受这两条影响。
...
| 步骤 | 做什么 |
|---|---|
| **1. 素材** | 粘贴 YouTube 链接或拖入本地文件。人声分离就在同一屏发起，三档质量任选。后台同时生成一份低分辨率代理视频，4K AV1 源也能顺畅拖动。引导声（ガイドメロディ）也在这一步做：五个旋钮，改完当场试听，不必等烧完片才发现不对。 |
...
| **4. 样式** | 配一条**有序字体链**（主字体缺哪个字，就用下面那个补上），几百个系统字体可按中英日任一写法搜索。按画面高度的百分比设字号 / 描边 / 阴影，同一套样式在 1080p 和 4K 下观感一致。每个声部四种颜色——未唱的填充与描边、已唱的填充与描边——因为描边是跟着填充一起翻色的。预览是成片画面的真实 libass 渲染，底色可切黑 / 绿 / 白。 |
| **5. 导出** | 选音轨、选要不要混入引导声——预览会把它放出来，勾选框不是盲赌。花几分钟烧片之前，关键位置条会把预览跳到最容易出问题的地方：第一句、每段开头、制作名单、最长的一行、注音最密的一行、末句。 |
...
- **随应用打包字体**——只能从系统已装字体里选；整条字体链的字形覆盖**在渲染前会检查**，
  并会告诉你每个字实际由链上哪个字体承担。
