...
lines = j["lines"]
print("总行数", len(lines))
multi = [(t["text"], t["start_ms"], t["dur_ms"]) for ln in lines for t in ln["tokens"]
         if len(t["text"]) != 1]
print("非单字符块:", multi)
...
print("正文块时长(ms) 中位/最小/最大 =",
      sorted(t["dur_ms"] for t in toks)[len(toks) // 2],
      min(t["dur_ms"] for t in toks), max(t["dur_ms"] for t in toks))
...
print("\n正文前 3 行完整:")
for ln in body[:3]:
    print("  [%d,%d]%s" % (ln["start_ms"], ln["dur_ms"],
                           "".join("%s(%d,%d)" % (t["text"], t["start_ms"], t["dur_ms"])
                                   for t in ln["tokens"])))
